...
/**
 * 通用交易所监控页面
 * 使用与 CryptoExchangePage 相同的布局
 * 数据来源会根据不同交易所的 ID 作为查询参数传递
 */
export default function GenericExchangePage() {
  const location = useLocation();
  // 从路径中提取交易所 ID (例如 /bybit -> bybit)
  const exchangeId = location.pathname.replace("/", "");
  
  return (
    <ExchangeProvider exchangeId={exchangeId}>
